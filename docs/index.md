# Performance benchmarking study of Open Source 5G Core networks
...
```bash
# Run the ansible role for OAI. Replace 172.24.4.223 with the IP of the OAI VM
...
First you need to install the tools for performance analysis as explaned in [this section](#how-to-set-up-performance-analysis-tools). We will make use of an ansible play on [this repo](https://github.com/tariromukute/control_plane_performance_analysis/blob/main/ansible/plays/oai.yml) to collect the results. The play will:
...
Visualise the results by using [this notebook](https://github.com/tariromukute/control_plane_performance_analysis/blob/main/notebooks/5GC-Performance-Analysis.ipynb) 
...
You will need to update the ip address in the files `config/free5gc-ue.yaml` and  `config/free5gc-gnb.yaml` on the core network VM.
...
You will need to update the ip address in the files `config/open5gs-ue.yaml` and  `config/open5gs-gnb.yaml` on the core network VM.
